...
func TestIssuerReconcile(t *testing.T) {
	type testCase struct {
		...
		expectedError                error
		expectedReadyConditionStatus essendixcissuerapi.ConditionStatus
	}
	...
	tests := map[string]testCase{
		"success-issuer": {
			...
			healthCheckerBuilder: func(*essendixcissuerapi.IssuerSpec, map[string][]byte) (signer.HealthChecker, error) {
				return &fakeHealthChecker{}, nil
			},
			expectedReadyConditionStatus: essendixcissuerapi.ConditionTrue,
		...
		"success-clusterissuer": {
			...
			issuerObjects: []client.Object{
				&essendixcissuerapi.ClusterIssuer{
					ObjectMeta: metav1.ObjectMeta{
						Name: "clusterissuer1",
					},
					Spec: essendixcissuerapi.IssuerSpec{
		...
		"issuer-missing-ready-condition": {
			...
			expectedReadyConditionStatus: essendixcissuerapi.ConditionUnknown,
		},
		"issuer-missing-secret": {
			...
			expectedReadyConditionStatus: essendixcissuerapi.ConditionFalse,
		},
	...
	require.NoError(t, essendixcissuerapi.AddToScheme(scheme))
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if condition != nil {
				...
				if reconcileErr != nil || condition.Status == essendixcissuerapi.ConditionFalse {
				...
				assert.Equal(
					t,
					[]string{fmt.Sprintf("%s %s %s", expectedEventType, essendixcissuerapi.EventReasonIssuerReconciler, eventMessage)},
					actualEvents,
...
func verifyIssuerReadyCondition(t *testing.T, status essendixcissuerapi.ConditionStatus, condition *essendixcissuerapi.IssuerCondition) {
