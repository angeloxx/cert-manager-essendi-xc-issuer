...
const (
	...
	defaultTaskCheckInterval   = time.Second * 15
)
...
// +kubebuilder:rbac:groups=essendixc.angeloxx.ch,resources=issuers;clusterissuers,verbs=get;list;watch
// +kubebuilder:rbac:groups=essendixc.angeloxx.ch,resources=issuers/status;clusterissuers/status,verbs=get;update;patch
// +kubebuilder:rbac:groups="",resources=secrets,verbs=get;list;watch
// +kubebuilder:rbac:groups="",resources=events,verbs=create;patch

func (r *IssuerReconciler) newIssuer() (client.Object, error) {
	issuerGVK := essendixcissuerapi.GroupVersion.WithKind(r.Kind)
...
func (r *IssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	report := func(conditionStatus essendixcissuerapi.ConditionStatus, message string, err error) {
		...
		r.recorder.Event(
			issuer,
			eventType,
			essendixcissuerapi.EventReasonIssuerReconciler,
			message,
		)
		issuerutil.SetReadyCondition(issuerStatus, conditionStatus, essendixcissuerapi.EventReasonIssuerReconciler, message)
	}

	// Always attempt to update the Ready condition
	defer func() {
		if err != nil {
			report(essendixcissuerapi.ConditionFalse, "Temporary error. Retrying", err)
		}
	...
	if ready := issuerutil.GetReadyCondition(issuerStatus); ready == nil {
		report(essendixcissuerapi.ConditionUnknown, "First seen", nil)
		return ctrl.Result{}, nil
	}
	...
	report(essendixcissuerapi.ConditionTrue, "Success", nil)
